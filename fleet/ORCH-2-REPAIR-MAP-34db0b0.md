...
**Superseded (2026-09-26T09:43:01Z).** This map is bound to `34db0b0` and is preserved verbatim, unedited. The current map —
`fleet/ORCH-2-REPAIR-MAP.md`, bound to `f5e2cf5` — was published at gate cycle L (CONTROL seq 56): ten of the thirteen
entries below closed on the worker's delivery, item 0g was re-classified by a disclosed gate amendment (defect #51), and
five rows remain open. §20 reads only the canonical path, so this file is a record, not the live list.
