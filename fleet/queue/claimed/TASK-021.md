# CLAIMED — TASK-021 (C1-drop threshold sensitivity over the v2 tuning half only)

claimant: WORKER-2 (A-2026-09-25-001), lane arena/01a0d9ce-fleetyard
claimed_utc: 2026-09-26T02:37:52Z (date -u at claim; the commit that carries this file is the source of record)
authority: ORCH-2 queue task (cut 2026-09-25T22:08:00Z; cycle-J update 2026-09-26T01:54:33Z sets
  criterion 21.8's binding floor at 257 and accepts either form of criterion 21.6)
preconditions: TASK-017 PASS (toolchain inherited); TASK-019b harness built, NOT run; split v2
  sealed and untouched (73d86f0d) — this task reads the v2 TUNING half only.
scoping decisions taken at claim time (recorded before any run):
  * the grid is executed as ONE bounded process per setting (LAW §4A): a runner may shard it, and
    each part carries its own exact run_utc + source, so no part's stamp is projected;
  * holdout discipline is by CONSTRUCTION (a scratch corpus of the 197 tuning transcripts only,
    book store symlinked) and by CODE (an audit hook over every `open` in the process), not by
    assertion — the manifest publishes the scratch listing digest, `holdout_reads: []`,
    `holdout_enforced: true` and the count of holdout files opened (0);
  * no threshold is chosen, fitted or recommended; counts only, CANDIDATE-class /
    PROVISIONAL-UNGATED; both denominators (v1 193 / v2 197) travel with every comparison to 122.
