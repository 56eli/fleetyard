...
def main(argv=None):
    ...
    prov = {
        ...
        "derivations": {
            ...
            "fixtures_digest_sha256":
                "sha256 over the joined SORTED lines '<sha256(file bytes)>  <path relative to "
                "--fixtures>\\n' for every .json file under --fixtures - i.e. "
                "fixtures/confirmed/confirmed.json and fixtures/confirmed/corrections.json "
                "(2 files; the directory is flat, so the keys ARE basenames). KEY CONVENTION: "
                "the records-dir-relative convention of records_digest_sha256 does NOT apply "
                "here. fixtures/clean/, fixtures/negative/ and fixtures/v2/ are OUTSIDE this "
                "binding, which is why adding fixtures/v2/dropword.json did not change "
                "c5d8f6f3... - the digest cannot prove that; the input set named here is the proof",
            "overlays_digest": "sha256(concat over basenames sorted of "
                               "'<sha256(text)>  <basename>\\n' for the 230 "
                               "corpus/docdocgo/overlays/*.txt), text decoded "
                               "utf-8/errors=replace; the LINES carry their newline and are "
                               "concatenated in basename order. WRONG VARIANT, constructed "
                               "exactly (do not use): sort the lines, strip each newline, "
                               "join with '\\n' and add no trailing newline -> 58274f46…",
            ...
            "json_canonicalization": "this manifest is written with json.dump(indent=1, "
                                     "sort_keys=True) (ensure_ascii default) and the committed "
                                     "file ends with one trailing newline; every M4 supplement "
                                     "config digest is sha256 over json.dumps(obj, "
                                     "sort_keys=True, separators=(',',':')) and states that in "
                                     "its own config_digest_note",
            "derivations_revision": {
                "corrected_by": "WORKER-2, TASK-020 item 13 (criterion 20.15)",
                "corrected_utc": "2026-09-26T01:45:17Z",
                "corrected_utc_source": "lane clock at write time; the commit carrying this "
                                        "text is the source of record (git log -S 'KEY "
                                        "CONVENTION' -- tools/m5r_reduce.py)",
                "why": "the run-time text was not literally executable: fixtures_digest_sha256 "
