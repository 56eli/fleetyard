...
# 0. Transport integrity
This letter traveled to you by git fetch from the hub repo — not by chat
paste — so it cannot have been truncated. Verify it anyway: `sha256sum
BOOTSTRAP-LETTER.md` must print exactly the sha the owner's launcher gave
you. Mismatch → STOP and report to the owner. Never reconstruct a missing
part from memory; ask for the launcher to be re-pasted.

You are an agent session inside `56eli/fleetyard` (public). You are bound to
your Arena session branch: you can commit and push ONLY to the branch your
session woke on (`git branch --show-current`). That is by design, and it is
enough — your branch is your LANE. This fleet is branch-agnostic: role
names (WORKER, ORCHESTRATOR, BOSS) are the addresses; branch names are
incidental and travel in boot prompts and final reports. `main` is the
owner's noticeboard — agents never write it, ever.

`main` currently carries ONE file: `docdocgo-fixes.zip` (16,116,395 bytes,
sha256 `3f36c520391049a49876d90e…` — verify yourself). Your job: unpack the
corpus, seed the fleet apparatus EXACTLY as specified ON YOUR OWN LANE,
report, and STOP. Do not start any role; the owner boots roles with
separate prompts.
...
## Step 2 — Seed the apparatus on YOUR lane (one commit)
...
- Skeletons (invent nothing): `fleet/LOG.md` (first line `# fleet log (append-only)`),
  ...
  `fleet/branches/SEED.md` — first line `# lane registry (append-only)`,
  then exactly two lines: `seed lane: <your branch name>` and
  `seeded: <UTC date>`.

## Step 3 — Lanes, not branches (read this; create nothing)
There are no `dev`/`review`/`boss` branches to create. Arena pins every
future role agent to its own session branch, and that branch IS its lane.
Each role agent, at boot: fetches the seed lane (your branch) READ-ONLY,
reads its role file from it, registers itself by committing
`fleet/branches/<ROLE>.md` on its OWN lane (branch name, UTC time, one-line
status), and from then on commits/pushes ONLY there. Any agent can find
lanes later: `git ls-remote origin`, then fetch a candidate `arena/*`
branch and read its `fleet/branches/<ROLE>.md` to identify it. Nothing ever
merges. `main` is never written by agents — the owner integrates by
checking out certified lane states.
...
3. `git ls-tree -r --name-only HEAD | wc -l`.
4. Your lane: `git branch --show-current` + `git rev-parse HEAD`; and
   `git rev-parse refs/remotes/origin/main` — UNCHANGED from session start
   (proof you never wrote main).
5. sha256 of each Appendix A–G file as committed.
6. `sha256sum BOOTSTRAP-LETTER.md` — echo the value you verified in step 0
   (letter-provenance record).

DONE = everything above pushed to YOUR lane + the battery + your lane name
and head sha reported. STOP.
